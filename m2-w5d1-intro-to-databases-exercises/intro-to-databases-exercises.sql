...
SELECT name, population FROM city WHERE district IN ('Ontario', 'Canada')
...
SELECT (gnp * 1000000 / population) FROM country WHERE continent = 'Europe'
-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT 2017 - indepyear FROM country WHERE indepyear IS NOT NULL 