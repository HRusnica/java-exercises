...
=======
		return null;
>>>>>>> f5985f8669c3512736831454fbd3d50c165f8a83
	}
...
<<<<<<< HEAD
		String jibberJabber = str.substring(str.length() - 2) + 
...
<<<<<<< HEAD
	 if (str.length() < 2) {
		...
		String ender = str.substring(str.length() - 2);
		if (ender.equals("ly")) {
		 return true;
	 } return false;
	}
...
<<<<<<< HEAD
		...
		if (badFirst.equals("bad")) {
			return true;
			} else if (str.length() > 3) {
		String badSecond = str.substring(1, 4);
		if (badSecond.equals("bad")) {
				return true;
				}
			} 
		}	return false;
...
<<<<<<< HEAD
		...
		String front = new String();
		front.equals(str.substring(0, 3));
		for (int i = 0; i < n; i++) {
